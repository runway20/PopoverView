...
- (void)showAtPoint:(CGPoint)point inView:(UIView *)view withText:(NSString *)text {
    ...
    label.textColor = [UIColor colorWithRed:0.329 green:0.341 blue:0.353 alpha:1];
    label.text = text;
    
    [self showAtPoint:point inView:view withContentView:[label autorelease]];
}

- (void)showAtPoint:(CGPoint)point inView:(UIView *)view withViewArray:(NSArray *)viewArray {
    UIView *container = [[UIView alloc] initWithFrame:CGRectZero];
    
    float totalHeight = kBoxPadding;
    float totalWidth = 2.f*kBoxPadding;
    ...
    for(UIView *view in viewArray) {
        ...
        view.frame = CGRectMake(kBoxPadding, totalHeight, view.frame.size.width, view.frame.size.height);
        ...
        if(view.frame.size.width + 2.f*kBoxPadding > totalWidth) {
            totalWidth = view.frame.size.width + 2.f*kBoxPadding;
        }
    ...
    for(UIView *view in viewArray) {
        ...
        view.frame = CGRectMake(kBoxPadding, totalHeight, totalWidth - 2.f*kBoxPadding, view.frame.size.height);
        ...
        if(kShowDividersBetweenViews && i != viewArray.count-1) {
            CGRect dividerRect = CGRectMake(view.frame.origin.x, view.frame.origin.y + view.frame.size.height, view.frame.size.width, 0.5f);
...
- (void)showAtPoint:(CGPoint)point inView:(UIView *)view withTitle:(NSString *)title withViewArray:(NSArray *)viewArray {
    ...
    UILabel *titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(kBoxPadding, 0, titleSize.width, titleSize.height)];
    ...
    float totalHeight = kBoxPadding + titleSize.height + titleHeightOffset;
    float totalWidth = 2.f*kBoxPadding + titleSize.width;
    ...
    for(UIView *view in viewArray) {
        //Now make sure all the labels are the full width
        view.frame = CGRectMake(view.frame.origin.x, view.frame.origin.y, totalWidth - 2.f*kBoxPadding, view.frame.size.height);
